	...
	protected void readNextMessage() {
		...
		try {
			...
			if (receivedBuffer[nbReceived++] == start) {
				...
				int crc = MAVLinkCRC.crc_calculate_decode(receivedBuffer,
						length);
				if (IMAVLinkCRC.MAVLINK_EXTRA_CRC) {
					// CRC-EXTRA for Mavlink 1.0
					crc = MAVLinkCRC
							.crc_accumulate(
									(byte) IMAVLinkCRC.MAVLINK_MESSAGE_CRCS[msgId],
									crc);
				}
				...
				if ((crcl == crcLow) && (crch == crcHigh)) {
					msg = MAVLinkMessageFactory.getMessage(msgId, sysId,
							componentId, rawData);
					if (msg != null) {
						...
						System.err.println("ERROR creating message  Id="
								+ msgId);
					}
				} else {
					System.err.println("ERROR mavlink CRC16-CCITT compute= "
							...
							+ Integer.toHexString(crcLow & 0x00FF) + " Id="
							+ msgId);
				}
				// restart buffer
				nbReceived = 0;
				...
				invalidData = true;
				// System.out.println("invalid : " + receivedBuffer[nbReceived -
				// 1]);
				// restart buffer
				nbReceived = 0;
			}
